...
const CategoryCards = async ({params} : {params : {slug : string , description : string , index : number }}) => {
  ...
  return (
    <div key={index} className="bg-pink-500 text-white w-80 h-45 rounded-xl p-3 flex flex-col items-start justify-between">
        ...
        <button className="font-bold text-md bg-red-900 text-white px-2 rounded-2xl py-1 curson-pointer hover:bg-white hover:text-pink-600" ><Link href={`/products/categories/${category}`}>Explore Now</Link></button>  
    </div>
  )
}
